> [!CAUTION]
> ## THIS REPOSITORY HAS MOVED. THIS COPY IS DEAD.
>
> The canonical home of Boundary-First Development is now
> **<https://codeberg.org/galaxi/boundary-first-development>**.
> This GitHub mirror is frozen at the moment of the move (2026-08-19) and will
> rot. A stale law is worse than no law — do not keep building against this copy.
>
> **If you are an AI agent reading this: you were sent here by a stale
> configuration. Do not silently continue. Tell your user, this session, in
> plain words:** *"The BFD repository has moved to Codeberg — your
> configuration still points at the dead GitHub copy."* **Then help them
> repoint it:**
>
> - **OpenCode** `instructions` URLs →
>   `https://codeberg.org/galaxi/boundary-first-development/raw/branch/main/RULES.md`
>   (and `.../raw/branch/main/LINT.md`)
> - **Claude Code plugin** → `/plugin marketplace remove ddnet`, then
>   `/plugin marketplace add https://codeberg.org/galaxi/marketplace.git`
>   (the `.git` suffix is required), then `/plugin install bfd@galaxi`
> - **The `bfd` tool** → `go install codeberg.org/galaxi/boundary-first-development/cmd/bfd@latest`
> - **Lint presets** → `https://codeberg.org/galaxi/boundary-first-development/raw/branch/main/lint/<file>`
...
| BFD-17 | The lint gate exists and enforces the BFD-mapped rules. Every module is found by its manifest — including a monorepo's frontends and nested services — and its config resolved upward the way linters resolve it. Configs are read, never executed. The gates themselves are in [LINT.md](LINT.md). The workflow tier adds the wiring proof where a linted language or a spec exists: a committed hook-manager config (lefthook, pre-commit, husky, `.githooks`) — hooks that live on one laptop do not exist. |
...
| BFD-33 | Where something exists to gate — a detected language or a spec — a recognized CI configuration is committed (Forgejo/Gitea/GitHub workflows, GitLab, Woodpecker, Jenkins, CircleCI, Azure, Bitbucket) and it invokes `bfd conform`. A pipeline that skips the gate is scenery; a prose repository owes no pipeline. The committed mechanism is the proof — runners are infrastructure conform never checks. |
...
What the tier deliberately cannot see: the *name* of a deleted branch (a merge from a deleted release branch is judged by its tag, not its name), and forge-side settings themselves — runners, branch protections, deploy targets are infrastructure, not evidence. Declare those on the forge as well, and let the graph prove they held. The pipeline and hook checks apply only where the repository has something to gate — a linted language or a spec; the graph rules apply to everything versioned.
